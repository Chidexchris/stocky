...
@section('title', 'Initialize New Tier')
@section('page_title', 'Configure Service Offering')

@section('content')
<div class="max-w-2xl mx-auto">
    <div class="bg-slate-800 rounded-2xl p-8 border border-white/5">
        <form action="{{ route('saas.plans.store') }}" method="POST" class="space-y-6">
            @csrf

            <!-- Name -->
            <div>
                <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Plan Identity</label>
                <input type="text" name="name" value="{{ old('name') }}" class="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-500" placeholder="e.g. Enterprise Elite" required>
            </div>
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" step="0.01" name="price" value="{{ old('price') }}" class="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-500" placeholder="0.00" required>
                </div>
                <div>
                    ...
                    <input type="number" step="0.01" name="price_annual" value="{{ old('price_annual') }}" class="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-500" placeholder="0.00" required>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="limit_stores" value="{{ old('limit_stores', 1) }}" class="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-500" required>
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Identity Limit (Users)</label>
                    <input type="number" name="limit_users" value="{{ old('limit_users', 1) }}" class="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-500" required>
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Storage Limit (GB)</label>
                    <input type="number" name="limit_storage" value="{{ old('limit_storage', 1) }}" class="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-500" required>
                </div>
                <div>
                    <label class="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">Currency Limit</label>
                    <input type="number" name="limit_currencies" value="{{ old('limit_currencies', 1) }}" class="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-500" required>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-blue-500" placeholder="Brief tagline or overview...">{{ old('description') }}</textarea>
            </div>
